...
status_t FFMPEGSoftCodec::setVideoFormat(
        status_t status,
        const sp<AMessage> &msg, const char* mime, sp<IOMXNode> omxNode, bool isEncoder,
    ...
    if (status != OK) {
        ...
        // compressionFormat will be override later
            ...
            *compressionFormat = (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingHEVC;
            ...
            err = setFFmpegVideoFormat(msg, omxNode);
            if (err != OK) {
    ...
    if (err == OK && (!strncmp(componentName, "OMX.qcom.", 9)
        ...
        if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_DIVX, mime) ||
            ...
            *compressionFormat = (OMX_VIDEO_CODINGTYPE)QOMX_VIDEO_CodingDivx;
            setQCDIVXFormat(msg, mime, omxNode, kPortIndexOutput);
        }
        ...
        if (msg->findInt32("thumbnail-mode", &thumbnailMode) &&
            ...
            status_t err = omxNode->getExtensionIndex(
            ...
            if (err != OK) {
                ...
                err = omxNode->setParameter(indexType,
...
status_t FFMPEGSoftCodec::setQCDIVXFormat(
        const sp<AMessage> &msg, const char* mime, sp<IOMXNode> omxNode, int port_index) {
    status_t err = OK;
...
status_t FFMPEGSoftCodec::getVideoPortFormat(OMX_U32 portIndex, int coding,
        sp<AMessage> &notify, sp<IOMXNode> omxNode) {
...
status_t FFMPEGSoftCodec::setAudioFormat(
        const sp<AMessage> &msg, const char* mime, sp<IOMXNode> omxNode) {
...
status_t FFMPEGSoftCodec::setWMVFormat(
        const sp<AMessage> &msg, sp<IOMXNode> omxNode)
{
    int32_t version = -1;
        ...
        const sp<AMessage> &msg, sp<IOMXNode> omxNode)
{
    ...
    status_t err = setRawAudioFormat(msg, omxNode);
    if (err != OK)
        return err;
